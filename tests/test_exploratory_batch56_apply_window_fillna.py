...
class TestComparisonTypeMismatch:
    ...
    def test_compare_with_lazy_aggregation(self):
        """Compare column to lazy aggregation result (known limitation).
        
        DataStore limitation: Using a lazy ColumnExpr aggregation directly in
        a filter condition generates invalid SQL (aggregate in WHERE clause).
        Use float(ds_df['col'].mean()) to force execution first.
        """
        pd_df = pd.DataFrame({'a': [1, 2, 3, 4, 5]})
        ds_df = DataStore(pd_df)
