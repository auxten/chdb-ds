...
class TestComplexMixedScenarios(unittest.TestCase):
    ...
    def test_where_after_sql_filter(self):
        ...
        # Verify where operation: rows with salary > 55000 should keep their values,
        # rows with salary <= 55000 should be replaced with 0
        rows_with_high_salary = 0
        rows_replaced_with_zero = 0

        for idx, row in df.iterrows():
            if row['salary'] > 55000:
                # These rows should have non-zero values
                self.assertGreater(row['id'], 0)
                self.assertGreater(row['age'], 0)
                rows_with_high_salary += 1
            else:
                # These rows should be all zeros (where replaced them)
                self.assertEqual(row['id'], 0)
                self.assertEqual(row['age'], 0)
                self.assertEqual(row['salary'], 0)
                rows_replaced_with_zero += 1

        # Verify we have both types of rows
        self.assertGreater(rows_with_high_salary, 0, "Should have rows with high salary")
        self.assertGreater(rows_replaced_with_zero, 0, "Should have rows replaced with zero")
    ...
    def test_alternating_sql_pandas_multiple_times(self):
        ...
        # Verify final state - all transformations should be applied
        # Check columns have the expected naming pattern
        # After: add_prefix('p1_') -> rename({'p1_id': 'final_id'}) -> add_suffix('_end')
        self.assertIn('final_id_end', df.columns)
        self.assertIn('p1_name_end', df.columns)
        self.assertIn('p1_age_end', df.columns)
        self.assertIn('p1_salary_end', df.columns)

        # All filters should have been applied
        # Original: age > 25, salary > 55000, id > 2
        # Expected IDs: 3, 5, 6, 7, 9 (age>25, salary>55000, id>2)
        self.assertTrue(all(df['p1_age_end'] > 25))
        self.assertTrue(all(df['p1_salary_end'] > 55000))
        self.assertTrue(all(df['final_id_end'] > 2))
        expected_ids = {3, 5, 6, 7, 9}
        actual_ids = set(df['final_id_end'])
    ...
    def test_chained_mathematical_operations(self):
        ...
        # Original salaries: 50000, 60000, 70000, 55000, 65000, 58000, 62000, 52000, 68000, 51000
        # Expected results: 101, 121, 141, 111, 131, 117, 125, 105, 137, 103
        expected_salaries = [
            (s * 2 + 1000) / 1000 for s in [50000, 60000, 70000, 55000, 65000, 58000, 62000, 52000, 68000, 51000]
        ]

        # Verify at least some values match the expected calculation
        for idx, row in df.iterrows():
            salary_result = row['salary']
            # The calculation should produce values in the expected range
            self.assertGreater(salary_result, 100)  # All should be > 100
            self.assertLess(salary_result, 142)  # Max should be 141
    ...
    def test_properties_after_mixed_operations(self):
        ...
        # Mix SQL and pandas
        ds_mixed = ds.select('*').filter(ds.age > 25).add_prefix('x_').drop(columns=['x_active'])
