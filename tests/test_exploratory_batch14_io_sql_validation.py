...
class TestSQLGenerationValidation:
    ...
    def test_groupby_agg_then_filter_sql(self):
        ...
        ds_exec = ds_result._execute()
        assert len(ds_exec) == len(pd_result)
        assert list(ds_exec.columns) == list(pd_result.columns)
...
class TestSpecialDataTypes:
    ...
    def test_timedelta_basic(self):
        """Test timedelta type."""
        pd_df = pd.DataFrame({
            'td': pd.to_timedelta([1, 2, 3, 4, 5], unit='D'),
            'val': [1, 2, 3, 4, 5]
        })
        ds = DataStore(pd_df)
...
class TestErrorHandling:
    ...
    def test_empty_dataframe_operations(self):
        ...
        assert list(ds_result._execute().columns) == ['a']
...
class TestComplexScenarios:
    ...
    def test_aggregation_with_multiple_functions(self):
        ...
        assert len(ds_agg._execute()) == len(pd_agg)
    ...
    def test_window_function_with_partition(self):
        ...
        ds_exec = ds_result._execute().reset_index(drop=True)
    ...
    def test_pivot_and_unpivot(self):
        ...
        # Compare shapes
        assert ds_pivot._execute().shape == pd_pivot.shape
...
class TestIndexOperations:
    ...
    def test_sort_index(self):
        ...
        assert list(ds_result._execute()['a']) == list(pd_result['a'])
...
class TestConcatBehavior:
    ...
    def test_concat_instance_method(self):
        ...
        assert len(ds_result._execute()) >= 2
...
class TestEdgeCases:
    ...
    def test_unicode_column_names(self):
        ...
        ds_result = ds._execute()
        assert '名前' in ds_result.columns or len(ds_result.columns) == 2
    ...
    def test_very_long_string(self):
        ...
        ds_result = ds._execute()
        assert len(ds_result.iloc[0]['a']) == 10000
