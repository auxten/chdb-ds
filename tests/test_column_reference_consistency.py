...
class TestComparisonOperationsMixed:
    ...
    def test_comparison_types(self):
        ...
        Expected:
        - ds["col"] > x returns ColumnExpr wrapping Condition (can materialize AND be used as condition)
...
class TestAggregationMixed:
    ...
    def test_aggregation_with_ds_getitem(self):
        ...
        try:
            stats = self.ds.groupby('flight_id').agg(
                avg_altitude=self.ds['altitude'].mean(),
                total_fuel=self.ds['fuel_consumed'].sum(),
            )
            result = stats.to_df()
    ...
    def test_aggregation_mixed_styles(self):
        ...
        try:
            stats = self.ds.groupby('flight_id').agg(
                avg_altitude=col('altitude').mean(),  # col() style
                total_fuel=self.ds['fuel_consumed'].sum(),  # ds["col"] style
                max_speed=self.ds.speed.max(),  # ds.col style
            )
            result = stats.to_df()
    ...
    def test_aggregation_result_consistency(self):
        """
        Verify that aggregation results are consistent across different reference methods
        """
        # Method 1: All col()
        stats1 = (
        ...
        # Results should be identical
        np.testing.assert_array_almost_equal(
            stats1['avg_alt'].values, stats2['avg_alt'].values, err_msg="col() vs ds['col'] aggregation results differ"
        )
        np.testing.assert_array_almost_equal(
            stats1['avg_alt'].values, stats3['avg_alt'].values, err_msg="col() vs ds.col aggregation results differ"
        )
...
class TestLazyExecutionConsistency:
    ...
    def test_multiple_operations_before_materialization(self):
        """
        Test behavior of multiple operations before materialization
        """
        ds = DataStore.from_df(self.df.copy())
        ...
        # Count lazy ops before materialization
        lazy_count_before = len(ds._lazy_ops)
        print(f"Lazy ops before materialization: {lazy_count_before}")
...
class TestRealWorldAviationScenarios:
    ...
    def test_fleet_statistics_by_aircraft_type(self):
        ...
        stats = self.ds.groupby('aircraft_type').agg(
            ...
            avg_fuel=self.ds['fuel_remaining_lbs'].mean(),
            avg_speed=self.ds.ground_speed_knots.mean(),
