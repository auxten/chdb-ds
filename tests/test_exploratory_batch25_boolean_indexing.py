...
class TestChainedAccessorOperations:
    ...
    def test_str_accessor_triple_chain(self):
        ...
        ds_df['result'] = ds_df['text'].str.strip().str.lower().str.replace('o', '0')
        ds_result = ds_df._get_df()['result']
    ...
    def test_dt_accessor_with_filter(self):
        ...
        ds_filtered['day_result'] = ds_filtered['date'].dt.day
        ds_result = ds_filtered._get_df()['day_result']
...
class TestGroupByNamedAggregation:
    ...
    def test_named_agg_with_index(self):
        ...
        pd_result = pd_df.groupby('A').agg(
            B_sum=('B', 'sum'),
            C_mean=('C', 'mean')
        )
        ds_result = ds_df.groupby('A').agg(
            B_sum=('B', 'sum'),
            C_mean=('C', 'mean')
        )
    ...
    def test_named_agg_with_as_index_false(self):
        ...
        pd_result = pd_df.groupby('A', as_index=False).agg(
            B_sum=('B', 'sum'),
            C_mean=('C', 'mean')
        )
        ds_result = ds_df.groupby('A', as_index=False).agg(
            B_sum=('B', 'sum'),
            C_mean=('C', 'mean')
        )
...
class TestSeriesOperationsViaAssign:
    ...
    def test_series_clip(self):
        ...
        ds_result = ds_df._get_df()['clipped']
    ...
    def test_series_between(self):
        ...
        ds_df['between'] = ds_df['A'].between(2, 4)
        ds_result = ds_df._get_df()['between']
        
        pd.testing.assert_series_equal(ds_result.reset_index(drop=True).astype(bool), 
                                       pd_result.reset_index(drop=True),
                                       check_names=False)
    ...
    def test_series_map_with_dict(self):
        ...
        ds_df['mapped'] = ds_df['A'].map(mapping)
        ds_result = ds_df._get_df()['mapped']
