...
class TestMultiColumnOperations:
    ...
    def test_select_computed_columns(self):
        ...
        pd_result = pd_df.assign(
            product=pd_df['a'] * pd_df['b'],
            ratio=pd_df['b'] / pd_df['a'],
            diff=pd_df['b'] - pd_df['a']
        )[['product', 'ratio', 'diff']]

        ds_result = ds_df.assign(
            product=ds_df['a'] * ds_df['b'],
            ratio=ds_df['b'] / ds_df['a'],
            diff=ds_df['b'] - ds_df['a']
        )[['product', 'ratio', 'diff']]
...
class TestSpecialValuesEdgeCases:
    ...
    def test_inf_in_arithmetic(self):
        ...
        pd_df = pd.DataFrame({
            'a': [1.0, np.inf, -np.inf, 0.0, 5.0],
            'b': [2.0, 3.0, 4.0, np.inf, -np.inf]
        })
        ds_df = DataStore(pd_df)
    ...
    def test_inf_filter(self):
        ...
        pd_df = pd.DataFrame({
            'a': [1.0, np.inf, -np.inf, 0.0, 5.0, np.inf],
            'b': [10, 20, 30, 40, 50, 60]
        })
        ds_df = DataStore(pd_df)
    ...
    def test_very_large_numbers(self):
        ...
        pd_df = pd.DataFrame({
            'a': [1e100, 1e200, 1e300, 1e50, 1e10],
            'b': [1e10, 1e20, 1e30, 1e40, 1e50]
        })
        ds_df = DataStore(pd_df)
    ...
    def test_very_small_numbers(self):
        ...
        pd_df = pd.DataFrame({
            'a': [1e-100, 1e-200, 1e-300, 1e-50, 1e-10],
            'b': [1e-10, 1e-20, 1e-30, 1e-40, 1e-50]
        })
        ds_df = DataStore(pd_df)
...
class TestStringEdgeCases:
    ...
    def test_string_replace_chain(self):
        ...
        pd_result = pd_df.assign(
            modified=pd_df['text'].str.replace('-', '_').str.upper()
        )
        ds_result = ds_df.assign(
            modified=ds_df['text'].str.replace('-', '_').str.upper()
        )
...
class TestMemoryLargeData:
    ...
    def test_large_dataframe_groupby(self):
        ...
        size = 10000
        pd_df = pd.DataFrame({
            'group': ['G' + str(i % 50) for i in range(size)],
