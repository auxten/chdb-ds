...
class TestAggregateFunctionsExecution(unittest.TestCase):
    ...
    def test_sum_sql(self):
        """Test sum() returns correct SQL."""
        sql = self.ds.select(self.ds['value'].sum().as_('total')).to_sql()
    ...
    def test_count_sql(self):
        """Test count() returns correct SQL."""
        sql = self.ds.select(self.ds['value'].count().as_('cnt')).to_sql()
        # count() may be count(*) or count("value") depending on implementation
        self.assertIn('count', sql.lower())

    def test_min_sql(self):
        """Test min() returns correct SQL."""
        sql = self.ds.select(self.ds['value'].min().as_('minimum')).to_sql()
        self.assertIn('min("value")', sql.lower())

    def test_max_sql(self):
        """Test max() returns correct SQL."""
        sql = self.ds.select(self.ds['value'].max().as_('maximum')).to_sql()
