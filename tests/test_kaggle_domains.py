...
class TestReadCSVBackendSelection:
    ...
    def test_true_false_values_uses_pandas_fallback(self, temp_dir):
        """true_values/false_values should fall back to pandas for correct behavior.

        ClickHouse's bool_true_representation/bool_false_representation settings
        are for OUTPUT format only, not for parsing input values. Therefore,
        we fall back to pandas when these parameters are specified to ensure
        correct boolean conversion behavior matching pd.read_csv().
        """
        ...
        # With true_values/false_values, should fall back to pandas
        result = ds.read_csv(filepath, true_values=['yes'], false_values=['no'])

        # Should use pandas (from_df), not chDB table function
        assert result._source_df is not None, "Should fall back to pandas with true_values/false_values"
        assert result._table_function is None, "Should NOT use chDB table function"

        # Verify data is correct (matches pandas behavior exactly)
        assert len(result) == 2
        import numpy as np

        np.testing.assert_array_equal(result.to_pandas()['active'].values, [True, False])
